function [Selected_Channel,ChannelNumber, RF_BPF] = choose_channel(Channels_IN,maxChannelNumber, fc0, fDelta, BW,Fs)
    ...
    Fc = fc0 + (ChannelNumber - 1) * fDelta;  % Carrier frequency (Hz)
    DeltaF = ceil(BW)/2;                       % Bandwidth (Hz)
    
    % Create the bandpass filter
    RF_BPF = createBandPassFilter(Fc, Fs, DeltaF,FilterOrder);
